function student_registration(id) {
    const form = document.getElementById('student_form' + id);
    const name = document.getElementById('name' + id).value.trim();
    const studentId = document.getElementById('student_id' + id).value.trim();
    ...
    const idError = document.getElementById('student_id_error' + id);
    ...
    // --- Validate Student ID ---
    if (!/^(?!0$)[0-9]{1,10}$/.test(studentId)) {
        idError.classList.remove('d-none');
        idError.innerHTML = "Student ID must be numeric (1–10 digits).";
        hasError = true;
    }
    ...
    fetch('/update_student_details', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, name, student_id: studentId })
    })
        .then(res => res.json())
        .then(data => {
            console.log(data);
...
function togglePassword(fieldId, id, button) {
...
function change_password_student(id) {
    const form = document.getElementById('student_form' + id);
